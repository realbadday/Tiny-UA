#!/usr/bin/env python3
"""
TinyLlama Chat Interface with TTS - Efficient Programming Assistant
Designed for CPU-only inference with text-to-speech support
...
class TinyLlamaChatTTS:
    """Efficient TinyLlama chat interface with TTS support"""
    
    def __init__(self, model_path=None, use_quantization=True, enable_tts=False):
        """
        Initialize TinyLlama chat with TTS
        Args:
    ...
    def generate_response(self, user_input, mode="chat", use_cache=True):
        """
        Generate response with CPU-optimized settings and TTS
        """
        # Check cache first
    ...
    def interactive_chat(self):
        ...
        while True:
            try:
                ...
                print(f"\n❌ Error: {e}\n")
                if self.tts and self.tts.enabled:
                    self.tts.speak(f"Error occurred")
...
- /tts-help    : Voice command details
"""
...
def main():
    ...
    parser.add_argument('--tts-rate', type=int, default=175,
                        help='TTS speech rate (50-300 WPM)')
    ...
    # Initialize chat
    chat = TinyLlamaChatTTS(
        model_path=args.model_path,
        use_quantization=not args.no_quantization,
        enable_tts=args.tts
    )
    
    # Set TTS parameters if enabled
    if chat.tts and chat.tts.enabled:
        chat.tts.set_rate(args.tts_rate)
        chat.tts.set_volume(args.tts_volume)
